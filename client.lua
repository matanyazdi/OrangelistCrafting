				...
				if inDrawingRange then
					...
					if not display and isPlayerInRange(GetEntityCoords(PlayerPedId()), Config.Recipes[i].coords, 2) then
						...
						if IsControlJustReleased(1, 38) then
							...
							if job ~= nil then
								if job == Config.Recipes[i].job then
									if grade >= Config.Recipes[i].grade then
										OpenWeaponStorage()
									else
										TriggerEvent('esx:showNotification', "Your job rank is too low")
									end
								else
									TriggerEvent('esx:showNotification', "You have no permission for the crafting")
								end
							else
								TriggerEvent('esx:showNotification', "You are not in job")
							end
						end
					end
				end
